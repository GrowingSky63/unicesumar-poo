...
class ProdutoView:
  def listar_produtos(self, produtos: list[Produto]) -> str | list:
...
class ProjetoView:
  def listar_projetos(self, projetos: list[Projeto]) -> str | list:
    lista = [f"ID: {p.get_id()} | Nome: {p.get_nome()}" for p in projetos]
    return lista if lista else "Nenhum projeto cadastrado."

  def mostrar_projeto(self, projeto: Projeto | None) -> str:
    if projeto is None:
      return "Projeto não encontrado."
    return f"ID: {projeto.get_id()} | Nome: {projeto.get_nome()}"


class TarefaView:
  def listar_tarefas(self, tarefas: list[Tarefa]) -> str | list:
